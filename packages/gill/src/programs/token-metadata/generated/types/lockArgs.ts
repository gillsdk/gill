...
export function getLockArgsEncoder(): Encoder<LockArgsArgs> {
  return getDiscriminatedUnionEncoder([
    ["V1", getStructEncoder([["authorizationData", getOptionEncoder(getAuthorizationDataEncoder())]])],
  ]);
}

export function getLockArgsDecoder(): Decoder<LockArgs> {
  return getDiscriminatedUnionDecoder([
    ["V1", getStructDecoder([["authorizationData", getOptionDecoder(getAuthorizationDataDecoder())]])],
  ]);
}
...
export function isLockArgs<K extends LockArgs["__kind"]>(kind: K, value: LockArgs): value is LockArgs & { __kind: K } {
