...
<Callout type="info">
  **What is the "use client" directive?** The `"use client"` directive tells the bundler that this
  ...
  which only work on the client - Solana RPC calls and wallet interactions need to happen in the
  browser - The provider creates React context, which is a client-side feature Without this
