...
<Callout type="info">
  **What is the "use client" directive?** The `"use client"` directive tells the bundler that this
  ...
  which only work on the client - The provider creates and hooks consume React context, which is a 
  client-side feature - Without this directive, you'll get errors about using hooks in Server Components.
</Callout>
