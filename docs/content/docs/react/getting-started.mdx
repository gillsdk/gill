---
title: Getting Started with gill-react
description: Learn how to set up and configure gill-react in your React application
---

This guide will walk you through setting up `gill-react` in your React application, including configuration for Next.js App Router, Remix, and other frameworks that support React Server Components.
...
First, install `gill-react` along with its peer dependencies:

```package-install
gill gill-react @tanstack/react-query
```
...
The `SolanaProvider` is a React context provider that makes the Solana client available to all gill-react hooks in your component tree.
...
<Callout type="info">
  **What is the "use client" directive?**
  ...
  - gill-react uses React hooks (useState, useEffect, etc.) which only work on the client
  - Solana RPC calls and wallet interactions need to happen in the browser
  - The provider creates React context, which is a client-side feature
...
### Use gill-react Hooks

Now you can use any gill-react hook in your components. When using frameworks with React Server Components (like Next.js App Router), remember to add the `"use client"` directive to any component that uses gill-react hooks:
...
gill-react is fully typed out of the box. No additional TypeScript configuration is required. However, ensure your `tsconfig.json` has:
...
If you see errors like "You're importing a component that needs useEffect. It only works in a Client Component" or "useState only works in Client Components", this means you're trying to use gill-react in a Server Component.

**Why this happens:**
- React Server Components run on the server during build/request time
- gill-react uses browser-specific features like React hooks and Web APIs
- The Solana client needs to make RPC calls from the browser

**To fix:**
1. Add `"use client"` at the top of any file using gill-react hooks
...
- Trying to use gill-react hooks directly in a page.tsx without the directive
...
Ready to start using gill-react? Check out the [available hooks](/docs/react/hooks) to begin fetching blockchain data in your React application.
