...
<Callout type="info">
  Transaction hooks are coming soon to gill-react. This page documents the planned API.
</Callout>
...
If you're using a wallet adapter, combine it with gill-react:
...
export function WalletTransaction() {
  ...
  const sendTransaction = async () => {
    ...
    const signature = await rpc
      .sendTransaction(signedTx, { skipPreflight: false })
      .send();
...
Transaction hooks are actively being developed. If you have suggestions or want to contribute, please check out the [gill-react repository](https://github.com/DecalLabs/gill) on GitHub.