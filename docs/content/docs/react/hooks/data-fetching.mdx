...
Get the account info for an address using the Solana RPC method [`getAccountInfo`](https://solana.com/docs/rpc/http/getaccountinfo).
...
Decode account data into a typed object:
...
Get an account's balance in lamports using [`getBalance`](https://solana.com/docs/rpc/http/getbalance).
...
Get the latest blockhash using [`getLatestBlockhash`](https://solana.com/docs/rpc/http/getlatestblockhash).
...
const { latestBlockhash } = useLatestBlockhash({
  commitment: "finalized", // Use finalized for transactions
});
...
Get the statuses of signatures using [`getSignatureStatuses`](https://solana.com/docs/rpc/http/getSignatureStatuses).
...
Get signatures for confirmed transactions using [`getSignaturesForAddress`](https://solana.com/docs/rpc/http/getsignaturesforaddress).
...
Get all accounts owned by a program using [`getProgramAccounts`](https://solana.com/docs/rpc/http/getProgramAccounts).
...
function RefreshButton() {
  ...
  const refreshAll = () => {
    // Invalidate all gill-react queries
